...
void ResultLogo3D::Init()
{
	...
	m_Model = new Model();
	m_Model->Load("asset\\model\\result.obj");
...
void ResultLogo3D::Update()
{
	// 絶対座標モードに変更
	Mouse_SetMode(MOUSE_POSITION_MODE_ABSOLUTE);

	// マウスの現在の状態を保持
	Mouse_GetState(&rMouse);

	if (Input::GetKeyTrigger(VK_LBUTTON))
	{
		m_MouseDownPosition.y = rMouse.y;
	}

	if (Input::GetKeyPress(VK_LBUTTON))
	{
		/*if (m_MouseDownPosition.y < tMouse.y)
		{
			m_MouseDiffernce.y = tMouse.y - m_MouseDownPosition.y;
			//m_MouseDiffernce.y = 100.0f;
			m_Position.y -= m_MouseDiffernce.y / 1000.0f;
		}*/

		m_Position.x = rMouse.x / 100.0f - 5.0f;
		m_Position.y = -rMouse.y / 100.0f + 5.0f;
		m_Velocity.x = 0.0f;
		m_Velocity.y = 0.0f;
	}
	else
