...
Model* SecondBullet::m_Model;//スタティックメンバ変数はcppで再度宣言が必要

#define BULLET_JUMP (0.10f)	// 弾の飛ぶ高さ

void SecondBullet::Load()
{
	m_Model = new Model();
	m_Model->Load("asset\\model\\bullet2.obj");
}

void SecondBullet::Unload()
{
	m_Model->Unload();
	delete m_Model;
}

void SecondBullet::Init()
{
