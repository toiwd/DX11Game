...
void Renderer::Init()
{
	...
	rasterizerDesc.CullMode = D3D11_CULL_BACK; // バックフェーズカリングの設定
	...
	m_DeviceContext->PSSetConstantBuffers(6, 1, &m_TimeBuffer);
	...
	LIGHT light{};
	light.Enable = true;
	light.Direction = D3DXVECTOR4(0.5f, -1.0f, -0.5f, 0.0f);
	D3DXVec4Normalize(&light.Direction, &light.Direction);
	light.Ambient = D3DXCOLOR(0.2f, 0.2f, 0.2f, 1.0f);
	light.Diffuse = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);
	//light.Position = D3DXVECTOR4(0.0f, 3.0f, 0.0f, 1.0f); // ライトの位置
	//light.Angle = D3DXVECTOR4(3.14159f / 180.0f*5.0f, 1.0f, 1.0f, 1.0f); // x=ライトの範囲（ラジアン）
	SetLight(light);
...
void Renderer::Begin()
{
	float clearColor[4] = { 0.8f, 0.1f, 0.1f, 1.0f };
...
{
	m_DeviceContext->UpdateSubresource(m_CameraBuffer, 0, NULL, &D3DXVECTOR4(CameraPosition.x, CameraPosition.y, CameraPosition.z, 1.0f), 0, 0);
}

void Renderer::SetTime(D3DXVECTOR4 Time)
{
