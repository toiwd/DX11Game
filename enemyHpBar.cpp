...
ID3D11Buffer*				EnemyHpBar::m_VertexBuffer = NULL;
ID3D11ShaderResourceView*	EnemyHpBar::m_Texture = NULL;
...
{
	m_Position = m_Pointer->GetPosition();
	float enemylife = m_Pointer->GetLife();
...
{
	// プレイヤーの状態を取得し
	bool enemystate = m_Pointer->GetEnemyState();
	float enemylife = m_Pointer->GetLife();
	...
	vertex[0].Position = D3DXVECTOR3(-1.0f, 2.5f, 0.0f);			//描画座標
	vertex[0].Normal = D3DXVECTOR3(0.0f, 1.0f, 0.0f);			//ライティング
	vertex[0].Diffuse = D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f);	//色
	vertex[0].TexCoord = D3DXVECTOR2(0.0f, 0.0f);				//テクスチャ座標

	vertex[1].Position = D3DXVECTOR3(-1.0f + enemylife / 50.0f, 2.5f, 0.0f);
	vertex[1].Normal = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
	vertex[1].Diffuse = D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f);
	vertex[1].TexCoord = D3DXVECTOR2(1.0f, 0.0f);

	vertex[2].Position = D3DXVECTOR3(-1.0f, 1.5f, 0.0f);
	vertex[2].Normal = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
	vertex[2].Diffuse = D3DXVECTOR4(0.0f, 0.0f, 1.0f, 1.0f);
	vertex[2].TexCoord = D3DXVECTOR2(0.0f, 1.0f);

	vertex[3].Position = D3DXVECTOR3(-1.0f + enemylife / 50.0f, 1.5f, 0.0f);
	vertex[3].Normal = D3DXVECTOR3(0.0f, 1.0f, 0.0f);
